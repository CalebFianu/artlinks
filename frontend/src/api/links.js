...
// --- CRUD ---
export const getLinks = (page = 1) => client.get(`/links/?page=${page}`);
...
export const getUserLinks = (username, page = 1) =>
  client.get(`/users/links/?username=${encodeURIComponent(username)}&page=${page}`);

export const getUserLinksByDay = (username, date) =>
  client.get(`/users/links/by_day/?username=${encodeURIComponent(username)}&date=${date}`);

export const getUserLinksByMonth = (username, month, year, page = 1) =>
  client.get(
    `/users/links/by_month/?username=${encodeURIComponent(username)}&month=${month}&year=${year}&page=${page}`
  );

export const getUserStats = (username) =>
  client.get(`/users/stats/?username=${encodeURIComponent(username)}`);

export const getUserFeaturedLinks = (username, page = 1) =>
  client.get(`/users/featured_links/?username=${encodeURIComponent(username)}&page=${page}`);
